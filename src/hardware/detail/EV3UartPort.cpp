...
namespace ev3lib::hardware::detail {
    ...
    EV3UartPort::EV3UartPort(EV3DeviceManager* manager, port_type port)
     ...
     */
    EV3UartPort::mode_type EV3UartPort::getMode() const {
     ...
     */
    bool EV3UartPort::setMode(mode_type mode) {
        // are we initialized ?
        if (m_modeInfo.empty())
    ...
    const volatile uint8_t* EV3UartPort::getData() const {
        auto uartData = getDevice().getSensorData();
#ifndef DISABLE_FAST_DATALOG_BUFFER
        return uartData->Raw[m_port][uartData->Actual[m_port]];
#else
        return uartData->Raw[m_port];
#endif
    }
    ...
    void EV3UartPort::read(std::span<uint8_t> buffer) {
        ...
        const volatile uint8_t* data = getData();
     ...
     */
    std::string EV3UartPort::getModeName(mode_type mode) const {
        if (mode < m_modeInfo.size()) {
            return m_modeInfo[mode].Name;
        }
     ...
     */
    int EV3UartPort::initSensor(mode_type mode) {
        int status;
        int retryCnt = 0;
     ...
     */
    bool EV3UartPort::initializeSensor(mode_type mode) {
        connect();
    ...
    bool EV3UartPort::readModeInfo() {
        ...
        for (mode_type i = 0; i < lms2012::MAX_DEVICE_MODES; ++i) {
            lms2012::UARTCTL uc = {0};
            if (getModeInfo(i, &uc)) {
                clearModeCache(i, &uc);
     ...
     * @param mode
     */
    void EV3UartPort::setOperatingMode(mode_type mode) {
        lejos::DEVCTL command;
        command.Port = m_port;
        command.Arg = mode;
     ...
     * @return
     */
    bool EV3UartPort::getModeInfo(mode_type mode, lms2012::UARTCTL* uc) {
        uc->Port = m_port;
        uc->Mode = mode;
     ...
     * @return
     */
    void EV3UartPort::clearModeCache(mode_type mode, lms2012::UARTCTL* uc) {
        uc->Port = m_port;
        uc->Mode = mode;
